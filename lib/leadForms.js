...
// company, website and subject only come from the /contact form
// (components/ContactForm.js); the shorter hero form omits them and their rows
// are dropped rather than left as empty dashes.
export async function sendContactEmail({ name, email, phone, message, company, website, subject, currentPage }) {
  if (!name || !email) {
  ...
  const optionalRows = [
    ...
    subject && row('Topic', escapeHtml(subject)),
  ]
    .filter(Boolean)
    .join('');
  ...
  const html = `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:640px;margin:0 auto;">
      ...
      <p style="font-size:13px;color:#8a8a8a;margin-bottom:20px;">Gobiya.com — Submitted from: ${escapeHtml(currentPage || 'Unknown page')}</p>
