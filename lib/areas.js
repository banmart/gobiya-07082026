// Canonical city data behind the location pages (/los-angeles-seo,
// /van-nuys-seo) and the AREA_SERVICES list they share.
...
// lib/heroImages.js rather than named here — the cities run 10–19, so
// neighbouring location pages never open onto the same photograph.
// Those files are 2752x1536, near enough
...
import { yearsInBusiness } from './authority';

// Studio City's copy claimed "16 years" in three places against the 2009
// founding date the rest of the site states. Derived from lib/authority.js so
// the figure cannot drift again, and ages on its own.
...
export const AREAS = [
  ...
  },
  {
    ...
    // Live route. See LIVE_AREAS at the foot of this file: a city with a
    // liveSlug renders at /<liveSlug>; every other city here is parked copy
    // whose old /areas-we-serve URL 301s to /los-angeles-seo.
    liveSlug: 'glendale-seo',
    // Four-position format: "Glendale SEO" leads the title, the description,
    // the h1, and the lede — the same phrase Search Console shows demand for
    // ("glendale seo" 27 impressions, "seo company glendale" 27, "seo
    // glendale" 18, "glendale seo company" 9, all stuck at position 56-61
    // while no page rendered this copy at all).
    h1: 'Glendale SEO That Turns Local Searches Into Customers',
    metaTitle: 'Glendale SEO Company That Turns Searches Into Customers',
    brandSuffix: false,
    metaDescription:
      'Glendale SEO from a team that started here in 1996 — local search, Google Business Profile, and content that earns rankings and AI citations alike.',
    lede:
      'Glendale SEO is the work of getting your business found by the people already searching for it on Brand Blvd, in the map pack, and in AI answers. Gobiya has done that work in Glendale longer than anywhere else: our founder grew up here, went to school here, and built his first websites here in 1996. That makes Glendale the one market we know as residents as well as consultants.',
    body: [
  ...
  },
  {
    ...
    heroEyebrow: 'Serving Hollywood Since 2009',
    heroExcerpt:
  ...
  },
  {
    ...
    details: [
      ...
      'Our white-hat, honest approach has helped Los Angeles Eastside businesses grow online since 2009.',
    ],
  },
  {
    slug: 'sherman-oaks',
    ...
    image: heroImage(15),
    excerpt: 'A Valley-local team, working Sherman Oaks since 2009.',
    desc: 'Sherman Oaks is a major commercial and residential hub in the San Fernando Valley. Gobiya has helped Sherman Oaks businesses recover from ranking drops and grow their digital presence since 2009.',
    details: [
  ...
  },
  {
    ...
    details: [
      ...
      'Our transparent, no-contract approach has made us a trusted partner for Silver Lake small businesses since 2009.',
    ],
  },
  {
    slug: 'studio-city',
    ...
    // Live route — see LIVE_AREAS at the foot of this file.
    liveSlug: 'studio-city-seo',
    // Four-position format: "Studio City SEO" leads the title, the
    // description, the h1, and the lede. The old title led with "Technical
    // SEO Services Studio City" and ran 81 characters, so Google truncated it
    // and the slug phrase never led anything. These are the page's four
    // measured queries: "seo company studio city" (position 20.4, the only
    // clicks the site earned), "studio city lead generation" (5.9), "studio
    // city technical seo services" (3.9), "seo services studio city" (18.2) —
    // so the title carries "SEO Company" and "Lead Generation" behind the
    // leading phrase rather than dropping them.
    metaTitle: 'Studio City SEO Company | Lead Generation That Books Work',
    brandSuffix: false,
    metaDescription:
      `Studio City SEO built around lead generation, not vanity metrics — technical SEO, Google Maps placement, and qualified calls. ${YEARS} years in the Valley.`,
    h1: 'Studio City SEO That Turns Local Searches Into Booked Work',
    lede:
      'Studio City SEO should be judged on booked work, not on where a keyword sat last Tuesday. Gobiya builds search campaigns for Studio City businesses around the things an owner can count — qualified calls, filled appointments, and jobs that close. That means fixing the technical faults that keep Google from reading your site, earning placement in the Studio City map pack, and writing pages that both Google and AI assistants trust enough to cite.',
    heroExcerpt:
    ...
    body: [
      ...
      { h3: 'Why Choose Gobiya SEO?' },
      {
        p: `Gobiya SEO has built a stellar reputation as a reliable, results-driven SEO services provider and digital marketing company in Studio City. Backed by ${YEARS} years of search engineering experience, we focus on delivering transparent, measurable growth.`,
      },
      {
    ...
    details: [
      ...
      `Our team has served Studio City and the surrounding Valley area for ${YEARS} years.`,
    ],
  },
  {
    slug: 'koreatown',
...
// The cities that render as real pages, at /<liveSlug>.
//
// Every city above is authored copy, but only the ones carrying a `liveSlug`
// have a route. The rest stay parked: their old /areas-we-serve/<slug> URL
// 301s to /los-angeles-seo, and nothing renders their body.
//
// The two that are live were chosen from Search Console rather than by
// guesswork — they are the only cities with measured demand, and between them
// they held 196 of the 250 impressions the retired city URLs were still
// earning, including both of the site's attributable clicks. Adding a city
// here means writing its four-position fields (metaTitle, metaDescription,
// h1, lede) first; without them the page would rank for nothing, which is the
// state the whole set was in before.
export const LIVE_AREAS = AREAS.filter((a) => a.liveSlug);

export function getLiveArea(liveSlug) {
  return LIVE_AREAS.find((a) => a.liveSlug === liveSlug) || null;
}

// Path for a city, live or parked — parked cities point at the page that
// absorbed their coverage claim so a link is never a dead end.
export function areaPath(area) {
  return area.liveSlug ? `/${area.liveSlug}` : '/los-angeles-seo';
}
