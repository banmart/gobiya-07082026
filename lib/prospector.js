...
export async function listProspects({ clientId, search, status, category, source, limit = 25, offset = 0 } = {}) {
...
export async function deleteProspects(ids) {
  if (!Array.isArray(ids) || ids.length === 0) {
    return { ok: false, error: 'No prospect ids provided.' };
  }
  const admin = createAdminSupabase();
  const { error, count } = await admin.from('prospects').delete({ count: 'exact' }).in('id', ids);
  if (error) {
...
// Deletes every prospect matching the given filters — used to clean up a whole bad
// import at once. Requires at least one real filter so it can't wipe the whole table.
export async function deleteProspectsByFilter({ clientId, search, status, category, source } = {}) {
  const hasFilter =
  ...
  if (!hasFilter) {
    return { ok: false, error: 'Refusing to delete without at least one filter applied.' };
  }

  const admin = createAdminSupabase();
  let query = admin.from('prospects').delete({ count: 'exact' });
