...
const SEVERITY_COLOR = { high: '#c8102e', medium: '#b7791f', low: '#475569' };
  ...
  return `
    <tr>
      <td style="padding:16px 0;border-bottom:1px solid #e5e7eb;">
        ...
        <p style="margin:0 0 6px;font-size:15px;font-weight:600;color:#0B1E36;">${escapeHtml(finding.title)}</p>
...
function buildHtml({ url, score, report, reportUrl, unmeasured }) {
  const scoreColor = score >= 80 ? '#15803d' : score >= 50 ? '#b7791f' : '#c8102e';
  ...
  <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:640px;margin:0 auto;padding:8px;">
    ...
    <h1 style="font-size:22px;font-weight:600;color:#0B1E36;margin:0 0 4px;">Your site scan results</h1>
    ...
    <h2 style="font-size:16px;font-weight:600;color:#0B1E36;margin:0 0 4px;">What we found</h2>
    <table style="width:100%;border-collapse:collapse;">${report.findings.map(findingBlock).join('')}</table>

    ${report.nextSteps?.length ? `
      <h2 style="font-size:16px;font-weight:600;color:#0B1E36;margin:28px 0 8px;">Where to start</h2>
    ...
    <div style="margin-top:28px;padding-top:20px;border-top:1px solid #e5e7eb;">
      <a href="${escapeHtml(reportUrl)}" style="display:inline-block;background:#0B1E36;color:#ffffff;font-size:14px;font-weight:600;padding:12px 22px;border-radius:6px;text-decoration:none;">View the full report</a>
      <p style="margin:16px 0 0;font-size:14px;color:#475569;">
        Questions? Reply to this email or call <a href="tel:+13237441338" style="color:#c8102e;">323-744-1338</a>. Steve answers both himself.
      </p>
    </div>
  </div>`;
}
